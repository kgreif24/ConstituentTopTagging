...
class Profile:
  ...
  def __init__ (self, title=None, depth=2):
    ...
    self.__endline   = None
    self.__prefix = "\033[38;2;74;176;245m\033[1m{title}\033[0m {icon} ".format(title=str(self.__class__).split(".")[-1], icon=termcolor.colored("", "red", attrs=["bold", "blink"]))
    self.__width = 80
  ...
  def prefix (self, **kwargs):
    ...
    return self.__prefix + self.indentString(**kwargs)
  ...
  def __enter__ (self):
    ...
    # Extract information
    frameinfo = self.getCallingFrameinfo()
