...
Once the files are prepared, generating a training set is a piece of cake: all that os left to do is to call the python script *mkdata.py* with your specific arguments. To get a list of all arguments just type (do not forget to source the virtual environment by calling `source bin/setup2p7.sh to avoid import errors):
...
The argument *truth-label* defines a selection that is given in the configuration file *selection.ini*. The following lines give an example of such a file:
...
[incl]
common=TMath::Abs(fjet_truthJet_eta)<2.0;fjet_truthJet_pt/1000>350
sig=TMath::Abs(fjet_truth_dRmatched_partrainticle_flavor)==6;TMath::Abs(fjet_truth_dRmatched_particle_dR)<0.75
bkg=
weight=fjet_testing_weight_pt_dR

[train]
common=fjet_m/1000.>40
